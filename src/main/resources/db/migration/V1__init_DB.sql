...
create table authors_books (author_id int8 not null, book_id int8 not null);
...
create table orders_details (id int8 not null, amount numeric(19, 2), price numeric(19, 2), book_id int8, order_id int8, details_id int8 not null, primary key (id));
create table publishing (id int8 not null, description varchar(255), name varchar(255), primary key (id));
create table users (id int8 not null, email varchar(255), name varchar(255), password varchar(255), role varchar(255), bucket_id int8, primary key (id));
alter table if exists authors_books add constraint unique_book_id unique (book_id);
alter table if exists orders_details add constraint unique_details_id unique (details_id);
alter table if exists users add constraint unique_email unique (email);
alter table if exists users add constraint unique_name unique (name);
alter table if exists authors_books add constraint authors_books_fk_books foreign key (book_id) references books;
alter table if exists authors_books add constraint authors_books_fk_authors foreign key (author_id) references authors;
alter table if exists books add constraint books_fk_authors foreign key (author_id) references authors;
alter table if exists books add constraint books_fk_publishing foreign key (publishing_id) references publishing;
alter table if exists books_genres add constraint book_genres_fk_genres foreign key (book_id) references genres;
alter table if exists books_genres add constraint books_genres_fk_books foreign key (genre_id) references books;
...
alter table if exists orders_details add constraint orders_details_fk_book_id foreign key (book_id) references books;
alter table if exists orders_details add constraint order_detail_fk_order_id foreign key (order_id) references orders;
